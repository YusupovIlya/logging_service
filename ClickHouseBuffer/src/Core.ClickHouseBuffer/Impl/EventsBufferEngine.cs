...
#pragma warning restore IDE0052 // Delete unread closed members
        ...
        {
            if (!Convert.ToBoolean(Environment.GetEnvironmentVariable("WriteToDb")))
            {
                _logger.LogInformation("Simulate write to db. Buffer has written {eventsCount} events to the database", events.Count());
